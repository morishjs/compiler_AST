...
private:
	...
	*/
	A_Identifier& ident;
	vector<A_Identifier*>* paramList;
public:
	A_Prototype(A_Identifier& id,vector<A_Identifier*>* il/*채워넣기*/) : ident(id), paramList(il) {}
...
public:
	A_Definition(/*채워넣기*/A_Prototype& h, A_Expr& b) : header(h), body(b) {}
	virtual void Print(int d);
};
...
private:
	...
	*/
	A_Identifier& ident;
public:
	A_VariableExpr(/*채워넣기*/A_Identifier &id) : ident(id) {}
	virtual void Print(int d);
};
...
private:
	...
	*/
	...
	A_TopList*		arguments;	//vector<A_TOP*> A_TOPlist
public:
	A_CallExpr(/*채워넣기*/A_Identifier& f, A_TopList* args) : funcName(f), arguments(args) {}
	virtual void Print(int d);
};
